...
for R1 in *R1*; do R2=${R1//R1_001.fastq.gz/R2_001.fastq.gz}; sample=${R1%_R1_001.fastq.gz}; STAR --runThreadN 8 --genomeDir ../../Whole_genome/GenomeDir --readFilesIn <(gunzip -c $R1 -2 $R2) --outSAMtype BAM Unsorted --quantMode TranscriptomeSAM --outFileNamePrefix $sample; done
